var logAbsen = new baseObject2({
	prefix : 'logAbsen',
	url : 'pages.php?page=logAbsen',
	formName : 'logAbsenForm',
	idFileCheck : 0,
	optionPushFile : 0,
	...
	Edit: function(){
		var errMsg = logAbsen.getJumlahChecked();
		urlEdit = this.url;
		if(errMsg == ''){
			$.ajax({
				type:'POST',
  ...
  if(errMsg == '' || errMsg=='Pilih hanya satu data'){
    swal({
        ...
        function(){
          $.ajax({
              type:'POST',
							data : $("#"+logAbsen.formName).serialize(),
				      url: logAbsen.url+'&API=Hapus',
              success: function(data) {
                var resp = eval('(' + data + ')');
                if(resp.err==''){
                  logAbsen.suksesAlert("Data Terhapus",logAbsen.daftarRender);
                }else{
                  logAbsen.errorAlert(resp.err);
                }
              }
	...
	saveNew: function(){
	  var me = this;
		...
		$.ajax({
			type:'POST',
			data : $("#"+logAbsen.formName+"_input").serialize(),
			url: logAbsen.url+'&API=saveNew',
				success: function(data) {
					me.closeLoading();
					var resp = eval('(' + data + ')');
						if(resp.err==''){
	...
	filterRender: function(){
		$.ajax({
			...
			url: logAbsen.url+'&API=filterRender',
			success: function(data) {
				var resp = eval('(' + data + ')');
				if(resp.err==''){
	...
	daftarRender: function(){
		logAbsen.showLoading();
		$.ajax({
			type:'POST',
			data : $("#formFilter").serialize(),
			url: logAbsen.url+'&API=daftarRender',
			success: function(data) {
				logAbsen.closeLoading();
				var resp = eval('(' + data + ')');
				if(resp.err==''){
