var logAdRequest = new baseObject2({
	prefix : 'logAdRequest',
	url : 'pages.php?page=logAdRequest',
	formName : 'logAdRequestForm',
	idFileCheck : 0,
	optionPushFile : 0,
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(resp.err==''){
					...
					logAdRequest.errorAlert(resp.err);
				}
			}
		});
	...
	Edit: function(){
		var errMsg = logAdRequest.getJumlahChecked();
		urlEdit = this.url;
		if(errMsg == ''){
			$.ajax({
				type:'POST',
			 ...
			 logAdRequest.errorAlert(errMsg);
		}
	},
	Hapus: function(){
  ...
  if(errMsg == '' || errMsg=='Pilih hanya satu data'){
    swal({
        ...
        function(){
          $.ajax({
              type:'POST',
							data : $("#"+logAdRequest.formName).serialize(),
				      url: logAdRequest.url+'&API=Hapus',
              success: function(data) {
                var resp = eval('(' + data + ')');
                if(resp.err==''){
                  logAdRequest.suksesAlert("Data Terhapus",logAdRequest.daftarRender);
                }else{
                  logAdRequest.errorAlert(resp.err);
                }
              }
	...
	homePage: function(){
		window.location = logAdRequest.url;
	},
	...
	saveNew: function(){
	  var me = this;
		...
		$.ajax({
			type:'POST',
			data : $("#"+logAdRequest.formName+"_input").serialize(),
			url: logAdRequest.url+'&API=saveNew',
				success: function(data) {
					me.closeLoading();
					var resp = eval('(' + data + ')');
						if(resp.err==''){
	...
	saveEdit: function(){
	  var me = this;
		...
		$.ajax({
			type:'POST',
			data : $("#"+logAdRequest.formName+"_input").serialize(),
			url: logAdRequest.url+'&API=saveEdit',
				success: function(data) {
					me.closeLoading();
					var resp = eval('(' + data + ')');
						if(resp.err==''){
	...
	filterRender: function(){
		$.ajax({
			...
			url: logAdRequest.url+'&API=filterRender',
			success: function(data) {
				var resp = eval('(' + data + ')');
				if(resp.err==''){
	...
	daftarRender: function(){
		...
		$.ajax({
			type:'POST',
			data : $("#formFilter").serialize(),
			url: logAdRequest.url+'&API=daftarRender',
			success: function(data) {
				logAdRequest.closeLoading();
				var resp = eval('(' + data + ')');
				if(resp.err==''){
