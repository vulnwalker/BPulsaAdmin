var logAdShow = new baseObject2({
	prefix : 'logAdShow',
	url : 'pages.php?page=logAdShow',
	formName : 'logAdShowForm',
	idFileCheck : 0,
	optionPushFile : 0,
	...
	Edit: function(){
		var errMsg = logAdShow.getJumlahChecked();
		urlEdit = this.url;
		if(errMsg == ''){
			$.ajax({
				type:'POST',
  ...
  if(errMsg == '' || errMsg=='Pilih hanya satu data'){
    swal({
        ...
        function(){
          $.ajax({
              type:'POST',
							data : $("#"+logAdShow.formName).serialize(),
				      url: logAdShow.url+'&API=Hapus',
              success: function(data) {
                var resp = eval('(' + data + ')');
                if(resp.err==''){
                  logAdShow.suksesAlert("Data Terhapus",logAdShow.daftarRender);
                }else{
                  logAdShow.errorAlert(resp.err);
                }
              }
	...
	saveNew: function(){
	  var me = this;
		...
		$.ajax({
			type:'POST',
			data : $("#"+logAdShow.formName+"_input").serialize(),
			url: logAdShow.url+'&API=saveNew',
				success: function(data) {
					me.closeLoading();
					var resp = eval('(' + data + ')');
						if(resp.err==''){
	...
	saveEdit: function(){
	  var me = this;
		...
		$.ajax({
			type:'POST',
			data : $("#"+logAdShow.formName+"_input").serialize(),
			url: logAdShow.url+'&API=saveEdit',
				success: function(data) {
					me.closeLoading();
					var resp = eval('(' + data + ')');
						if(resp.err==''){
	...
	filterRender: function(){
		$.ajax({
			...
			url: logAdShow.url+'&API=filterRender',
			success: function(data) {
				var resp = eval('(' + data + ')');
				if(resp.err==''){
	...
	daftarRender: function(){
		...
		$.ajax({
			type:'POST',
			data : $("#formFilter").serialize(),
			url: logAdShow.url+'&API=daftarRender',
			success: function(data) {
				logAdShow.closeLoading();
				var resp = eval('(' + data + ')');
				if(resp.err==''){
