var settingAd = new baseObject2({
	prefix : 'settingAd',
	url : 'pages.php?page=settingAd',
	formName : 'settingAdForm',
	idFileCheck : 0,
	optionPushFile : 0,
	...
	Edit: function(){
		var errMsg = settingAd.getJumlahChecked();
		urlEdit = this.url;
		if(errMsg == ''){
			$.ajax({
				type:'POST',
  ...
  if(errMsg == '' || errMsg=='Pilih hanya satu data'){
    swal({
        ...
        function(){
          $.ajax({
              type:'POST',
							data : $("#"+settingAd.formName).serialize(),
				      url: settingAd.url+'&API=Hapus',
              success: function(data) {
                var resp = eval('(' + data + ')');
                if(resp.err==''){
                  settingAd.suksesAlert("Data Terhapus",settingAd.daftarRender);
                }else{
                  settingAd.errorAlert(resp.err);
                }
              }
	...
	saveNew: function(){
	  var me = this;
		...
		$.ajax({
			type:'POST',
			data : $("#"+settingAd.formName+"_input").serialize(),
			url: settingAd.url+'&API=saveNew',
				success: function(data) {
					me.closeLoading();
					var resp = eval('(' + data + ')');
						if(resp.err==''){
	...
	saveEdit: function(){
	  var me = this;
		...
		$.ajax({
			type:'POST',
			data : $("#"+settingAd.formName+"_input").serialize(),
			url: settingAd.url+'&API=saveEdit',
				success: function(data) {
					me.closeLoading();
					var resp = eval('(' + data + ')');
						if(resp.err==''){
	...
	filterRender: function(){
		$.ajax({
			...
			url: settingAd.url+'&API=filterRender',
			success: function(data) {
				var resp = eval('(' + data + ')');
				if(resp.err==''){
	...
	daftarRender: function(){
		settingAd.showLoading();
		$.ajax({
			type:'POST',
			data : $("#formFilter").serialize(),
			url: settingAd.url+'&API=daftarRender',
			success: function(data) {
				settingAd.closeLoading();
				var resp = eval('(' + data + ')');
				if(resp.err==''){
