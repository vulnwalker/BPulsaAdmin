<?php
class logAbsen extends baseObject{
  var $Prefix = "logAbsen";
  var $formName = "logAbsenForm";
  var $tableName = "absen";
  var $username = "";
  function jsonGenerator(){
    ...
    foreach ($_POST as $key => $value) {
      ...
      case 'Edit':{
        $content = $this->Edit($logAbsen_cb[0]);
  		break;
  		}
      case 'saveNew':{
  			if(empty($namaMember)){
      ...
      case 'Hapus':{
        for ($i=0; $i < sizeof($logAbsen_cb) ; $i++) {
          $query = "delete from $this->tableName where id = '".$logAbsen_cb[$i]."'";
          $this->sqlQuery($query);
        }
        $cek = $query;
        break;
  ...
  function loadScript(){
    return "
    <script type='text/javascript' src='js/logAbsen/logAbsen.js'></script>
                ...
                <div class='modal-body'>
                  ...
                  <div class='form-group'>
                    <div class='row'>
                      ...
                      <div class='col-sm-9'>
                        ".$this->cmbArray("statusAbsen",$getDataEdit['status'],array(array("PENDING","PENDING"),array("SUKSES","SUKSES")),"-- STATUS --","class='form-control'")."
                      </div>
                    </div>
                  </div>
                </div>
  ...
  function setKolomHeader(){
    ...
    <thead>
      <tr>
          ...
          <th style='text-align:center;width:50%;'>Nama Member </th>
  ...
  function setKolomData($no,$arrayData){
    ...
    <tr class='$classRow'>
        ...
        <td style='text-align:center;'>".$this->generateDate($tanggal)."</td>
  ...
  function generateTable($kondisiTable){
    ...
    $getDataServer = $this->sqlQuery("select * from absen $kondisiTable ");
