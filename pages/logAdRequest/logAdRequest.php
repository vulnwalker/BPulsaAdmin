<?php
class logAdRequest extends baseObject{
  var $Prefix = "logAdRequest";
  var $formName = "logAdRequestForm";
  var $tableName = "log_request_ad";
  var $username = "";
  function jsonGenerator(){
    ...
    foreach ($_POST as $key => $value) {
      ...
      case 'Edit':{
        $content = $this->Edit($logAdRequest_cb[0]);
  		break;
  		}
      case 'saveNew':{
  			if(empty($namaMember)){
      ...
      case 'Hapus':{
        for ($i=0; $i < sizeof($logAdRequest_cb) ; $i++) {
          $query = "delete from $this->tableName where id = '".$logAdRequest_cb[$i]."'";
          $this->sqlQuery($query);
        }
        $cek = $query;
        break;
  ...
  function loadScript(){
    return "
    <script type='text/javascript' src='js/logAdRequest/logAdRequest.js'></script>
  ...
  function generateTable($kondisiTable){
    ...
    $getDataServer = $this->sqlQuery("select * from log_request_ad $kondisiTable ");
