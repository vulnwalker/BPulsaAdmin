<?php
class refPayment extends baseObject{
  var $Prefix = "refPayment";
  var $formName = "refPaymentForm";
  ...
  function jsonGenerator(){
    ...
    foreach ($_POST as $key => $value) {
      ...
      case 'Edit':{
        $content = $this->Edit($refPayment_cb[0]);
  		break;
  		}
      case 'saveNew':{
  			if(empty($namaMember)){
      ...
      case 'Hapus':{
        for ($i=0; $i < sizeof($refPayment_cb) ; $i++) {
          $query = "delete from $this->tableName where id = '".$refPayment_cb[$i]."'";
          $this->sqlQuery($query);
        }
        $cek = $query;
        break;
  ...
  function loadScript(){
    return "
    <script type='text/javascript' src='js/refPayment/refPayment.js'></script>
      ...
      <div class='hide' id='popover-search' >
        <div class='pad5A '>
            <div class='form-group'>
              ...
              <div class='row' style='margin-top:5px !important;'>
                <label class='col-sm-1 control-label' style='margin-top:6px;'>Penukaran</label>
                <div class='col-sm-11'>
                  ".$this->textBox(array(
                    "id" => 'filterPenukaran',
                    "value" => $filterPenukaran,
                  ))."
                </div>
              </div>
              <div class='row' style='margin-top:5px !important;'>
              ...
              <div class='row' style='margin-top:5px !important;'>
                <label class='col-sm-1 control-label' style='margin-top:6px;'>Status</label>
                <div class='col-sm-11'>
                  ".$this->cmbArray("filterStatus",$filterStatus,array(array("","-- STATUS --"),array("PENDING","PENDING"),array("SUKSES","SUKSES")),"-- STATUS --","class='form-control'")."
                </div>
              </div>
              <div class='row' style='margin-top:5px !important;'>
  ...
  function setKolomHeader(){
    ...
    <thead>
      <tr>
          ...
          <th style='text-align:center;width:10%;'>Penukaran</th>
  ...
  function setKolomData($no,$arrayData){
    ...
    $getDataTradePoint = $this->sqlArray($this->sqlQuery("select * from trade_point where id = '$id_trade_point'"));
    $tableRow = "
    <tr class='$classRow'>
        <td style='text-align:center;'>$no</td>
        <td style='text-align:center;'>".$this->setCekBox($no - 1,$id,$this->Prefix)."</td>
        <td>".$getDataMember['nama']."</td>
        <td>".$getDataTradePoint['title']."</td>
  ...
  function generateTable($kondisiTable){
    ...
    $getDataServer = $this->sqlQuery("select * from payment $kondisiTable ");
  ...
  function generateCondition(){
    ...
    if(!empty($filterPenukaran)){
      $arrayIdTradePoint = array();
      $getDataTradePoint = $this->sqlQuery("select * from trade_point where title like '%$filterPenukaran%'");
      while ($dataTradePoint = $this->sqlArray($getDataTradePoint)) {
        $arrayIdTradePoint[] = $dataTradePoint['id'];
      }
      $arrKondisi[] = " id_trade_point in (".implode(",",$arrayIdTradePoint).") ";
    }
